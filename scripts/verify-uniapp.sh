...
if rg -q '@VERSION@|@CHECKSUM@' "${marketplace_path}" || \
   ! rg -q "${actual_checksum}" "${marketplace_path}"; then
...
ruby -rjson -e '
  ...
  unsupported = client.fetch("uni-app-x")
  values = lambda do |value|
    value.is_a?(Hash) ? value.values.flat_map { |entry| values.call(entry) } : [value]
  end
  abort "uni-app x must be explicitly unsupported" unless values.call(unsupported).all? { |entry| entry == "x" }
...
runtime_aar="${package_root}/utssdk/app-android/libs/LevixelUniRuntime-${version}.aar"
