...
dcloud_ios_sdk_root="${DCLOUD_IOS_SDK_ROOT:-${repo_root}/../dcloud/5.07/SDK}"
...
archive_name="sandrox-levixel-uniapp-${version}.zip"
...
if [[ ! -f "${dcloud_ios_sdk_root}/SDK/inc/DCUni/DCUniModule.h" ]]; then
  echo "DCloud iOS SDK not found: ${dcloud_ios_sdk_root}" >&2
  exit 1
fi
...
"${plugin_dir}/native/android/gradlew" \
  ...
  :levixel-uniapp:testReleaseUnitTest \
  :levixel-uniapp:assembleRelease \
  :levixel-uniapp:copyUniappBundledAars \
...
photoview_aar="$(find "${android_adapter_dir}/levixel-uniapp/build/uniapp-bundled-aars" -maxdepth 1 -type f -name '*.aar' -print -quit)"
